...
export default function MFIApplicationsPage() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to load applications.";
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  const openActionModal = (id: string, action: "approve" | "reject", name: string) => {
  ...
  const tabs: { key: Tab; label: string; icon: React.ElementType; activeColor: string }[] = [
    { key: "all", label: "All", icon: ClipboardList, activeColor: "text-blue-600 bg-blue-50 border-blue-300" },
    { key: "pending", label: "Pending", icon: Clock, activeColor: "text-amber-600 bg-amber-50 border-amber-300" },
    { key: "approved", label: "Approved", icon: CheckCircle2, activeColor: "text-emerald-600 bg-emerald-50 border-emerald-300" },
    { key: "rejected", label: "Rejected", icon: XCircle, activeColor: "text-destructive bg-destructive/10 border-destructive/30" },
  ];
        ...
        <div className="relative rounded-[2rem] overflow-hidden bg-primary p-8 md:p-10 text-primary-foreground shadow-xl shadow-primary/20">
          ...
          <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-2">
              ...
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">Loan Applications</h1>
              <p className="text-primary-foreground/70 text-sm max-w-md leading-relaxed">
                Review and manage all incoming loan applications from entrepreneurs across the platform.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 p-1.5 bg-muted rounded-2xl overflow-x-auto w-full md:w-auto">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon size={16} className={isActive ? tab.activeColor.split(" ")[0] : ""} />
                {tab.label}
              </button>
            );
          })}
        ...
        <CardContent className="p-6 min-h-[400px]">
          {loading && applications.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
              ...
              <Button variant="outline" size="sm" className="mt-2 rounded-xl" onClick={fetchApplications}>
                Try Again
              </Button>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-xs leading-relaxed">
                We couldn't find any applications matching your current filters and search criteria.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {applications.map((app) => {
                ...
                return (
                  <div key={app.id} className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 p-5 rounded-2xl bg-background border border-border hover:border-primary/30 hover:shadow-sm transition-all duration-200 group">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      {app.status === "pending" && (
                        <div className="flex gap-2 mr-2 border-r border-border pr-5">
                          <Button 
                            ...
                            onClick={() => openActionModal(app.id, "reject", app.applicant_name)}
                          >
                            <X size={14} /> Reject
                          </Button>
                          <Button 
                            ...
                            onClick={() => openActionModal(app.id, "approve", app.applicant_name)}
                          >
                      ...
                      <span className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider border", status.className)}>
                        <StatusIcon size={12} />
                        {status.label}
                      </span>

                      <Link href={`/mfi/applications/${app.id}`}>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors" title="View Details">
                          <ArrowRight size={16} />
                        </Button>
                      </Link>
                    </div>
